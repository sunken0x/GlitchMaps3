// Glitch Maps - Restored Speed Logic for GitHub Pages

console.log("Script Start - Restoring Speed Logic");
...
let R; try { R = new Random(); } catch(e) { console.error("RNG Init Error:", e); R = { /* Fallback R */ };}
...
class GlitchWave {
    ...
    fractalNoise(x, y, time) {
        // <<< CORRECTED: Ensure Medium speed exists, use this.params.speedMultiplier >>>
        ...
        const timeFactor = time * currentSpeedMult / mediumSpeed;

        const octave1 = Math.sin(x * 0.5 + timeFactor * 0.003) * Math.cos(y * 0.3 + timeFactor * 0.004);
        let result = octave1;
        const octaves = this.params.chaos?.octaves ?? 1;
        ...
        if (!this.lines?.length) { return; } try {
            ...
            this.lines.forEach((lD)=>{
                 ...
                 for(let i=0; i<oP.length; i++){
                     ...
                     let w=0;
                     // <<< CORRECTED: Apply speedMultiplier consistently to wave patterns >>>
                     const mediumSpeed = (this.params.speeds && typeof this.params.speeds['Medium'] === 'number') ? this.params.speeds['Medium'] : 0.002;
                     const currentSpeedMult = typeof this.params.speedMultiplier === 'number' ? this.params.speedMultiplier : mediumSpeed;
                     const timeFactor = time * currentSpeedMult / mediumSpeed; // Time adjusted by speed

                     // Apply timeFactor to wave calculations
                     if(TRAITS.wavePattern==='Circular Ripple'){const d=Math.sqrt(p.x*p.x+p.y*p.y); w=Math.sin(timeFactor*0.1-d*0.6)*2;}
                     else if(TRAITS.wavePattern==='Linear Wave'){w=Math.sin(timeFactor*0.1+p.x*2)*2;}
                     else if(TRAITS.wavePattern==='Diagonal Sweep'){w=Math.sin(timeFactor*0.1+(p.x+p.y)*1.5)*2;}
                     else if(TRAITS.wavePattern==='Radial Burst'){const a=Math.atan2(p.y,p.x); w=Math.sin(timeFactor*0.2+a*3)*2;}
                     else if(TRAITS.wavePattern==='Cross Pattern'){w=Math.sin(timeFactor*0.1+p.x*2)*Math.cos(timeFactor*0.1+p.y*2)*2;}
                     else if(TRAITS.wavePattern==='Square Wave'){w=Math.sin(timeFactor*0.1+p.x*1.5)*2.2;}
                     else if(TRAITS.wavePattern==='Sawtooth Wave'){const ph=(timeFactor*0.1+p.x*2)%(Math.PI*2); w=((ph/Math.PI)-1)*2;}
                     else if(TRAITS.wavePattern==='Zigzag Pattern'){const d1=Math.sin(timeFactor*0.1+(p.x+p.y)*1.5); const d2=Math.sin(timeFactor*0.1+(p.x-p.y)*1.5); w=(d1+d2)*1.2;}
                     else if(TRAITS.wavePattern==='Concentric Squares'){const dx=Math.abs(p.x); const dy=Math.abs(p.y); const mD=Math.max(dx,dy); w=Math.sin(timeFactor*0.1-mD*0.8)*2;}
                     else if(TRAITS.wavePattern==='Smooth Center'){const d=Math.sqrt(p.x*p.x+p.y*p.y); w=Math.sin(timeFactor*0.16-d*2.0)*2.5;}

                     // Pass original time to noise function, it applies multiplier internally
    ...
    setTimeout(() => {
        try {
             if (glitchWaveInstance?.renderer?.domElement) {
                 ...
                 console.log('✅ Full GM Code (Fixed Scope + Restored Speed) ready');
...
console.log("artwork.js: Script End");
